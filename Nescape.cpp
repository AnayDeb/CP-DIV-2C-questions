...
/*
   Observations:-
   -> One main observation is if the maximum values are present in 
   neighbour then we can press that buttons with one clone only else
   we need to make another clone.

   -> The main observation is that we can treat a chunk of same numbers
   as one , as it can affect our answer in many ways.

   -> Rest the implementation is easy.
*/
void solve() {
    int n;
    cin>>n;
