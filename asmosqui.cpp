...
/*
   Observations:-
   -> One observation i got is that if we try to make the gifts greedily
   more ,then we can do that.

   -> Another observation is that if there are more than 1 odds then we
   can take the max odd number where we will maximize the sum and the
   other odd numbers we will make them even with operation with the 
   other even numbers,making the even numbers to 1 and odd number as 
   big as possible.
*/
void solve() {
    ll n; 
